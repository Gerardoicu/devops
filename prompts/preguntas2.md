...
#### Pregunta 78:Estás trabajando como ingeniero DevOps en una empresa de comercio electrónico y tienes desplegada una aplicación Node.js en Elastic Beanstalk. Te gustaría rastrear las tasas de error y específicamente necesitas asegurarte, observando los logs de la aplicación, de que no tengas más de 100 errores en un intervalo de 5 minutos. En caso de que estés recibiendo demasiados errores, te gustaría ser alertado vía email.
...
#### Pregunta 80:El equipo DevOps en una empresa líder de billetera bitcoin y servicios de exchange está intentando desplegar una plantilla de CloudFormation que contiene una función Lambda, un bucket S3, un rol IAM y una tabla DynamoDB desde CodePipeline, pero el equipo está obteniendo una InsufficientCapabilitiesException.
...
#### Pregunta 82:Una empresa minorista multinacional está planificando recuperación ante desastres y necesita que los datos se almacenen en Amazon S3 en dos regiones diferentes que estén en distintos continentes. Los datos se escriben a una alta tasa de 10000 objetos por segundo. Por razones regulatorias, los datos también deben estar encriptados en tránsito y en reposo. La empresa te ha contratado como AWS Certified DevOps Engineer Professional para construir una solución para este requerimiento.
...
#### Pregunta 83:Como parte del pipeline CI/CD, un ingeniero DevOps está realizando una prueba funcional usando una plantilla de CloudFormation que posteriormente será desplegada a producción. Esa plantilla de CloudFormation crea un bucket S3 y una función Lambda que transforma imágenes cargadas en S3 en miniaturas. Para probar la función Lambda, algunas imágenes se cargan automáticamente y se espera la salida de miniaturas desde la función Lambda en el bucket S3. Como parte de la limpieza de estas pruebas funcionales, el stack de CloudFormation se elimina, pero en este momento falla la eliminación.
...
#### Pregunta 87:Una empresa de comercio electrónico desea automatizar el parcheo de su flota híbrida y distribuir algunos parches a través de sus repositorios internos de parches cada semana. Como ingeniero DevOps en la empresa, se te ha encargado implementar esto de la forma más eficiente posible.
...
#### Pregunta 88:El departamento de cumplimiento en una firma de trading de Wall Street te ha contratado como AWS Certified DevOps Engineer Professional para ayudar con varias iniciativas estratégicas DevOps. El departamento te ha pedido que generes una lista de todos los paquetes de software instalados en las instancias EC2. La solución debe ser capaz de extenderse a futuras instancias en la cuenta AWS y enviar notificaciones si las instancias no están configuradas correctamente para rastrear su software.
...
#### Pregunta 89:Una empresa de analítica está capturando métricas de sus servicios y aplicaciones en AWS usando métricas de CloudWatch. Necesita poder retroceder hasta 7 años en el tiempo para visualizar estas métricas debido a requisitos regulatorios. Como ingeniero DevOps en la empresa, se te ha encargado diseñar una solución que ayude a la empresa a cumplir con las regulaciones.
...
#### Pregunta 91:Una empresa de comercio electrónico está gestionando todo su stack de aplicación e infraestructura usando AWS OpsWorks Stacks. El equipo DevOps ha notado que muchas instancias han sido reemplazadas automáticamente en el stack y el equipo desea ser notificado vía Slack cuando estos eventos ocurran.
...
#### Pregunta 94:El equipo DevOps en una empresa líder de servicios de reservas de viajes está usando una plantilla de CloudFormation para desplegar una función Lambda. El código de la función Lambda se carga en S3 en un archivo llamado s3://my-bucket/my-lambda-code.zip por CodePipeline después de haber pasado todas las verificaciones de build requeridas. CodePipeline invoca la plantilla de CloudFormation para desplegar el nuevo código. El equipo ha encontrado que, aunque la plantilla de CloudFormation se ejecuta exitosamente, la función Lambda no se actualiza.
...
#### Pregunta 96:Como Lead DevOps Engineer en una empresa de comercio electrónico, deseas actualizar la versión mayor de tu base de datos MySQL, la cual está gestionada por CloudFormation con AWS::RDS::DBInstance y configurada en Multi-AZ.
...
#### Pregunta 97:El equipo DevOps en tu empresa está usando CodeDeploy para desplegar nuevas versiones de una función Lambda después de que ha pasado CodeBuild vía CodePipeline. Antes de desplegar, CodePipeline tiene un paso que opcionalmente inicia una reestructuración de archivos en un bucket S3 que es compatible hacia atrás. Esta reestructuración se realiza usando una ejecución de Step Function que invoca una tarea en Fargate. La nueva función Lambda no puede funcionar hasta que la tarea de reestructuración haya sido completamente finalizada.
...
#### Pregunta 99:Una empresa minorista está finalizando su migración a AWS y se da cuenta de que mientras algunos empleados han aprobado la certificación AWS Certified DevOps Engineer Professional y conocen bien AWS, otros aún están comenzando y ni siquiera han obtenido certificaciones de nivel Associate. La empresa ha establecido reglas arquitectónicas y de etiquetado específicas para recursos internos y desea minimizar el riesgo de que empleados principiantes creen recursos no conformes.
...
#### Pregunta 100:Una empresa de gaming desea poder recibir notificaciones casi en tiempo real cuando la llamada API DeleteTable es invocada en DynamoDB.
...
#### Pregunta 101:Una empresa minorista usa Jenkins open-source en su infraestructura on-premise para realizar CI/CD. Ha decidido migrar a AWS y aprovechar las propiedades de elasticidad del cloud para tener cargas de trabajo más eficientes. Necesita asegurar que la configuración de Jenkins sea altamente disponible, tolerante a fallos y también elástica para ejecutar builds. La empresa te ha contratado como AWS Certified DevOps Engineer Professional para construir la solución más costo-efectiva para este requerimiento.
...
Una empresa de analitica de Big Data ha desplegado una aplicacion de procesamiento en streaming utilizando KCL para leer registros desde Kinesis Data Streams configurado con multiples shards. La aplicacion se esta ejecutando en una instancia EC2. Parece que la aplicacion consumidora se esta quedando atras bajo una carga grande y, por lo tanto, los registros no se procesan a tiempo y eventualmente se pierden del stream.

Como ingeniero DevOps, se te ha encargado mejorar la confiabilidad de esta aplicacion con cambios minimos, ?que harias? (Selecciona dos)

Respuestas:

A. Incrementar el numero de shards en Kinesis para aumentar el throughput

B. Ejecutar la aplicacion en un Auto Scaling Group y escalar basado en la metrica de CloudWatch MillisBehindLatest

C. Disminuir el numero de shards en Kinesis para reducir la carga

D. Aumentar el periodo de retencion de datos del stream

E. Migrar la aplicacion a AWS Lambda

Explicacion general

Opciones correctas:

Ejecutar la aplicacion en un Auto Scaling Group y escalar basado en la metrica de CloudWatch MillisBehindLatest

En una arquitectura tipica de Kinesis Data Streams, tienes productores que continuamente envian datos a Kinesis Data Streams, y los consumidores procesan los datos en tiempo real. Los consumidores (como una aplicacion personalizada ejecutandose en Amazon EC2 o un delivery stream de Amazon Kinesis Data Firehose) pueden almacenar sus resultados usando un servicio de AWS como Amazon DynamoDB, Amazon Redshift o Amazon S3.

via - https://docs.aws.amazon.com/streams/latest/dev/key-concepts.html

Conceptos clave para Kinesis Data Streams:
via - https://docs.aws.amazon.com/streams/latest/dev/key-concepts.html

La Kinesis Client Library (KCL) asegura que para cada shard exista un procesador de registros ejecutandose y procesando ese shard. La libreria tambien simplifica la lectura de datos desde el stream. La Kinesis Client Library usa una tabla de Amazon DynamoDB para almacenar datos de control.

Para el caso planteado, necesitas ejecutar KCL en multiples instancias EC2 detras de un Auto Scaling Group. Ejecutar mas procesos de KCL es la clave aqui, y para ello necesitamos un Auto Scaling Group basado en la metrica MillisBehindLatest, que representa el tiempo que el iterador actual esta retrasado respecto al registro mas reciente (la punta) en el shard. La Kinesis Client Library (KCL) publica metricas personalizadas en Amazon CloudWatch en tu nombre, usando el nombre de tu aplicacion KCL como namespace.

via - https://docs.aws.amazon.com/streams/latest/dev/monitoring-with-kcl.html

Aumentar el periodo de retencion de datos del stream

El periodo de retencion es el tiempo durante el cual los registros de datos permanecen accesibles despues de ser a?adidos al stream. El periodo de retencion de un stream se establece por defecto en 24 horas despues de su creacion. Para evitar que los registros se pierdan, es recomendable aumentar el tiempo de retencion y permitir un mayor margen para procesarlos. El maximo periodo de retencion que puedes configurar es de 7 dias.

Opciones incorrectas:

Migrar la aplicacion a AWS Lambda - Migrar la aplicacion a AWS Lambda no ayudara con el tiempo de procesamiento, ya que eventualmente el mismo codigo de procesamiento se ejecutaria ya sea en EC2 o en Lambda.

Incrementar el numero de shards en Kinesis para aumentar el throughput - Incrementar el numero de shards en Kinesis puede aumentar el throughput total del stream, pero esto no impacta el rendimiento de procesamiento de tus procesos (que depende de lo que haces con los mensajes). Incrementar los shards podria ayudarte a aumentar el numero de procesos de KCL si eso ya fuera el limite, pero actualmente solo tenemos un proceso de KCL ejecutandose, por lo que no esta al maximo de su capacidad.

Disminuir el numero de shards en Kinesis para reducir la carga - Disminuir el numero de shards reduciria el throughput, pero nuevamente no tendria efecto en el rendimiento de la aplicacion consumidora.
...
#### Pregunta 103:Una empresa multinacional ha definido lineamientos y estándares de etiquetado para todos sus recursos en AWS y desea crear un dashboard para visualizar el cumplimiento de todos los recursos con la capacidad de encontrar los recursos no conformes. La empresa te ha contratado como AWS Certified DevOps Engineer Professional para desarrollar una solución para este requerimiento.
